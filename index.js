...
async function getWeather() {
  try {
    ...
    let res = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=ba98deab6ce9cd74af9735fb0625c520&units=metric`
    );
    if (res == undefined) {
...
function appendsData(data) {
  ...
  document.getElementById("rise").innerHTML = `${window
    .moment(data.sys.sunrise * 1000)
    .format("HH:mm a")}`;
  document.getElementById("set").innerText = `${window
  ...
  async function Wheather(lat, lon) {
    try {
      let daily = await fetch(
        `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=,hourly,minutely&units=metric&appid=ba98deab6ce9cd74af9735fb0625c520`
      );
...
setInterval(function () {
  ...
  document.querySelector("#Rightdate").innerHTML =
    days[day] + ", " + date + " " + months[month];
}, 1000);
...
function nightFun() {
  document.getElementById(
    "right"
  ).style.backgroundImage = `url("https://images.unsplash.com/photo-1504608524841-42fe6f032b4b?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8OXx8d2VhdGhlcnxlbnwwfHwwfHw%3D&auto=format&fit=crop&w=500&q=60")`;
  document.getElementById(
    "left"
  ).style.backgroundImage = `url("https://images.pexels.com/photos/2403402/pexels-photo-2403402.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500")`;
  ...
  document.getElementById(
    "rightOne"
  ).style.backgroundImage = `url("https://images.pexels.com/photos/4100130/pexels-photo-4100130.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500")`;
}

function dayFun() {
  document.getElementById(
    "rightOne"
  ).style.backgroundImage = `url("https://img.freepik.com/free-photo/sunset_1127-3503.jpg?t=st=1647101732~exp=1647102332~hmac=92cb9920daf4d948b338ed6e15f9bfd7239e87dcefafee34bef5addabea67553&w=360")`;
  document.getElementById("right").style.backgroundColor = "white";
  document.getElementById(
    "left"
  ).style.backgroundImage = `url("https://img.freepik.com/free-vector/gorgeous-clouds-background-with-blue-sky-design_1017-25501.jpg?size=626&ext=jpg&ga=GA1.1.1608378299.1647101724")`;
  document.getElementById(
    "right"
  ).style.backgroundImage = `url("https://images.pexels.com/photos/1784577/pexels-photo-1784577.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500")`;
